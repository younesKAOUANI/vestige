 ...
 *
 * <p>The severity each {@link ConditionType} looks at is fixed by the type itself ({@link
 * ConditionType#NEW_CRITICAL_ISSUES} always means CRITICAL-or-above, {@link
 * ConditionType#TOTAL_BLOCKER_ISSUES} always means BLOCKER-or-above) rather than being a free
 * parameter. That is what keeps the gate a small closed set instead of an expression language — see
 * {@link ConditionType}.
 *
    ...
    public String describe() {
        return switch (type) {
            case NEW_CRITICAL_ISSUES ->
                    "new issues of severity >= CRITICAL <= %d".formatted(threshold);
