 ...
 *
 * <p>The set is deliberately small and closed rather than an expression language. A gate a
 ...
 * scoped to <b>new code</b> — issues the matcher opened or reopened in the run being evaluated.
 * §7 is explicit that this scope is "defined by the matcher, not by the diff": there is
 * deliberately no separate "on a changed line" restriction here (ADR-008). {@link
 * #TOTAL_BLOCKER_ISSUES} is scoped to the whole project on this branch, which is what makes it
 * catch debt that predates the current run.
 */
