 ...
 */
public record GateConfigRequest(
        @NotBlank String name, @NotEmpty List<@Valid ConditionRequest> conditions) {
    ...
    public QualityGateDefinition toDefinition() {
        return new QualityGateDefinition(
                name,
                conditions.stream().map(c -> new GateCondition(c.type(), c.threshold())).toList());
    }
}
