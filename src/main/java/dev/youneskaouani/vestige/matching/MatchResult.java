 ...
 *     new Issue}
 * @param noLongerPresent previous issues no current finding matched - §3.3's
 *     {@code for p in unmatched_P: p.status <- RESOLVED_FIXED (auto)}
 */
public record MatchResult(
        List<Match> matches, List<IncomingFinding> newIssues, List<PreviousIssueCandidate> noLongerPresent) {
