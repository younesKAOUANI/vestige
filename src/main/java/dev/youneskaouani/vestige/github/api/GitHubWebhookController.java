 ...
 *
 * <p>Bypasses {@code ApiKeyAuthenticationFilter} by path (see its {@code
 * UNAUTHENTICATED_PREFIXES}) because GitHub cannot present an {@code X-API-Key}; the signature
 * check below is this endpoint's authentication, not an addition on top of one.
 *
 * <p>Verification is real and tested ({@link WebhookSignatureVerifier}). Acting on the verified
 * payload - re-triggering a re-baseline when a PR's base branch changes, decorating a PR the
 * moment its check run is known rather than waiting for the next poll - is not: v1 verifies and
 * logs every event and stops there. See README "Roadmap"; nothing about the ingestion or matching
 * pipeline depends on this endpoint doing more, since {@code POST /api/v1/runs} is what CI actually
 * calls to submit a report.
 */
@RestController
...
public class GitHubWebhookController {
    ...
    public ResponseEntity<Void> receive(
        ...
        if (secret == null || secret.isBlank()) {
            throw new Problems.Forbidden("GitHub webhook integration is not configured on this instance");
        }
        ...
        // TODO(Roadmap): act on push/pull_request events (re-baseline, PR decoration) instead of only
