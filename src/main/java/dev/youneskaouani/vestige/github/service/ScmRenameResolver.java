...
/**
 * Resolves file renames between two commits (§3.2's "File rename handling"), so the previous run's
 * findings can be re-fingerprinted against a file's <em>current</em> path before matching.
 *
...
public interface ScmRenameResolver {
     ...
     *     provider reports as renamed (not merely edited) between the two commits. Empty - never an
     *     exception - when there is no base commit to compare against, or when the provider is
     ...
     */
    Map<String, String> renamesBetween(
