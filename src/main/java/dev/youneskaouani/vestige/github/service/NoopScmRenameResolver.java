 ...
 * {@code GitHubConfig}). Matching still works without it - identity_fp and context_fp do not
 * depend on the rename map at all, and weak_fp only needs it for the specific case of a file that
 * both moved <em>and</em> changed enough to defeat the other two rungs - so running without GitHub
