 ...
 *
 * <p>GitHub's compare API works unauthenticated for public repositories, so a blank token would not
 * strictly have to mean "never call it" - but making an outbound call to a third-party API by
 * default, for an operator who never configured GitHub integration at all, is the wrong default. A
 * blank token is treated as "GitHub integration is off" across the board (renames here, Check Runs
 * in the gate module), which is the simpler contract to document and to reason about.
 *
 ...
 * a subtlety not worth the risk in code that (see README, "A note on how this was built") could not
 * be compiled against the real Spring context to check.
 */
@Configuration
public class GitHubConfig {

    @Bean
    public ScmRenameResolver scmRenameResolver(
            RestClient.Builder restClientBuilder, VestigeProperties properties) {
