...
public interface AuditChainHeadRepository extends JpaRepository<AuditChainHead, UUID> {
     ...
     * locking method rather than adding {@code @Lock} to {@code findById}, so that a read that
     * does not need the lock (e.g. {@code GET /api/v1/audit/verify}) never silently takes one. See
     * {@link AuditChainHead}'s class javadoc for why this lock is what serialises concurrent
     * triage safely.
     */
