 ...
 * happens here - only {@link SarifReader#peekToolIdentity} runs on the request thread, which is
 * the whole reason that method exists (see its own javadoc). The rest of §4.2's pipeline is
 * {@code RunProcessingService}'s job, off this thread entirely.
 */
@Service
public class RunIngestionService {
    ...
    public sealed interface SubmissionResult {
        ...
        /** The idempotency key matched an identical earlier submission. The caller answers {@code 200}. */
    ...
    public SubmissionResult submit(
        ...
        if (sarif.length > maxBytes) {
            // ReportSizeFilter already rejects most oversized uploads from Content-Length alone; this
            // is the fallback for a chunked request that filter cannot see the size of in advance.
            throw new Problems.PayloadTooLarge(
                    "Report is %d bytes, exceeding the %d byte limit".formatted(sarif.length, maxBytes));
        }
        ...
        String idempotencyKey = (clientIdempotencyKey != null && !clientIdempotencyKey.isBlank())
                ...
                : Sha256.hexOfFields(project.getId().toString(), commitSha, tool.name(), reportDigest);
        ...
        Optional<AnalysisRun> existing = runRepository
                .findByOrganizationIdAndIdempotencyKey(organizationId, idempotencyKey)
                .or(() -> runRepository.findByProjectIdAndCommitShaAndAnalyserNameAndReportDigest(
                        project.getId(), commitSha, tool.name(), reportDigest));
        if (existing.isPresent()) {
            return resolveDuplicate(existing.get(), project.getId(), branch.getId(), commitSha, reportDigest);
        }
        ...
        // supplied by either) loses the analysis_run_natural_key_idx/analysis_run_idempotency_key_idx
        // unique index and surfaces to its own caller as a generic 409 via
        // GlobalExceptionHandler#handleDataIntegrityViolation, rather than the 200-with-original-result
        // §4.1 describes for a repeat key. That is a deliberate, narrow simplification: the race window
        // is milliseconds wide, and the losing client's correct response to a 409 here is the same
        // "retry the identical request" it would already do for any other transient conflict, at which
        // point it lands on the now-existing row and gets its 200. A seamless 200-on-race would need a
        // catch-and-re-read loop around this method, which is more machinery than a millisecond-wide
    ...
    private Project resolveProject(
            UUID organizationId, String provider, String owner, String repo, String branchName, Instant now) {
        return projectRepository
                .findByProviderAndOwnerAndName(provider, owner, repo)
                .orElseGet(() -> projectRepository.save(
                        new Project(UUID.randomUUID(), organizationId, provider, owner, repo, branchName, now)));
    }
     ...
     */
    private Branch resolveBranch(UUID organizationId, Project project, String branchName, Instant now) {
    ...
    private SubmissionResult resolveDuplicate(
            AnalysisRun existing, UUID projectId, UUID branchId, String commitSha, String reportDigest) {
        boolean sameRequest = existing.getProjectId().equals(projectId)
