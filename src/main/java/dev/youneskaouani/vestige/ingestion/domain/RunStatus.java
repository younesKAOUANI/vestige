 ...
 * terminal state once the outbox worker has exhausted its retries (see
 * {@code V1__core_schema.sql}'s comment on {@code analysis_run.status} and
 * {@code AnalysisJobRepository}).
 *
 ...
 * before committing - a run that completes normally jumps straight from {@code RECEIVED} to
 * {@code COMPLETED}. They exist as real states anyway, rather than being collapsed into one
 * generic {@code PROCESSING}, because a crash-recovery read (or a support engineer looking at a
 * stuck run) benefits from knowing which half of the transaction was in flight.
 *
